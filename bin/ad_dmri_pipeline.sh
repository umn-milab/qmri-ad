...
if [ ! -d $MPRFOLDER ];then
	mkdir -p $MPRFOLDER
    chmod -R 770 $MPRFOLDER
fi

ad_dmri_process.sh $NIIFOLDER/$SUB/dwi $DMRI79RESULT/$SUB $SUB dmri $MODFILEFOLDER $STAGE $MAXCPUS $DSICPUS $CUDAVERSION $NIIFOLDER/$SUB/anat $MPRFOLDER/$SUB
